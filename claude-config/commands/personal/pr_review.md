...
- Use casual messaging (e.g., "What do you think about...")
...
### Step 6: Prepare gh CLI Commands / MCP Commands
...
4. Add each line comment to the pending review (one call per comment):
   ...
   If the GitHub MCP server is available, prefer using `add_comment_to_pending_review`.
...
6. **After all comments are added**, submit the review as an approval:
   ...
   If the GitHub MCP server is available, prefer using `pull_request_review_write`.
